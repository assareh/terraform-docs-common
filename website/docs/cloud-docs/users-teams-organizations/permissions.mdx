...
- Delete the workspace
  - Depending on the [organization's settings](/cloud-docs/users-teams-organizations/organizations#general), workspace admins may only be able to delete the workspace if it is not actively managing infrastructure. Refer to [Deleting a Workspace With Resources Under Management](/cloud-docs/workspaces/settings#deleting-a-workspace-with-resources-under-management) for details.
...
This permission implicitly gives permission to read runs on all workspaces, which is necessary to set enforcement of [policy sets](/cloud-docs/policy-enforcement/manage-policy-sets).
...
Allows members to create and administrate all projects within the organization. This grants the following abilities:

- Admin access to all projects.
- Admin access to all workspaces (effectively the same permissions as "Manage Workspaces").
- The ability to create new projects.
